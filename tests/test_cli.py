...
def test_shots_groupsSameNameFilesAndSkipsSortedFolders(tmp_path):
    _touch(
        ...
        tmp_path / "day2" / "sharp" / "DSC0007.JPG",  # only top-level sorted folders are skipped
        tmp_path / "sharp" / "DSC0005.JPG",  # sorted on an earlier run
    )
    shots = _shots(tmp_path)
    assert [s.photo.relative_to(tmp_path) for s in shots] == [
        Path("DSC0001.JPG"),
        Path("day2/DSC0002.jpg"),
        Path("day2/sharp/DSC0007.JPG"),
    ]
...
def test_move_takesSidecarsAndRefusesToOverwrite(tmp_path):
    ...
    _touch(photo, raw)
    assert _move(Shot(photo, [photo, raw]), tmp_path / "sharp")
    assert sorted(p.name for p in (tmp_path / "sharp").iterdir()) == ["DSC0001.ARW", "DSC0001.JPG"]

    _touch(photo)
    assert not _move(Shot(photo, [photo]), tmp_path / "sharp")
    assert photo.exists()
