...
_HUB_AUTH_NAG = "unauthenticated requests to the HF Hub"
...
def species_crop(image: Image.Image, bird: Bird) -> Image.Image:
    """Square crop around the bird with some context, clamped to the frame.

    Shrunk right away so a batch of crops stays small; the classifier sees 224 px anyway.
    """
    x0, y0, x1, y1 = bird.box
    side = min(round(max(x1 - x0, y1 - y0) * (1 + 2 * CROP_PAD)), image.width, image.height)
...
class SpeciesClassifier:
    def __init__(self, device: torch.device):
        # Public weights need no token; drop only the Hub's X-HF-Warning nag about it.
        logging.getLogger("huggingface_hub.utils._http").addFilter(
            lambda record: _HUB_AUTH_NAG not in record.getMessage()
        )
