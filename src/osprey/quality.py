"""Subject sharpness: how blurry the bird itself is, ignoring bokeh backgrounds."""
...
MAX_SIDE = 1024  # judge sharpness at screen-viewing scale, never upscale
BLUR_WINDOW = 11
...
def bird_sharpness(image: Image.Image, bird: Bird) -> float:
    """Sharpness of the bird's own pixels, at most MAX_SIDE across."""
    crop = image.crop(bird.box).convert("L")
    mask = Image.fromarray(bird.mask)
    scale = min(1.0, MAX_SIDE / max(crop.size))
    if scale < 1:
        size = (round(crop.width * scale), round(crop.height * scale))
        crop, mask = crop.resize(size, Image.LANCZOS), mask.resize(size, Image.NEAREST)
    return sharpness(np.asarray(crop), np.asarray(mask))
