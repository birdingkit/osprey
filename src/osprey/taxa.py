...
# The API drops connections now and then; retry those and 429/5xx with 1-2-4-8 s backoff.
_session = requests.Session()
_session.mount(
    "https://", HTTPAdapter(max_retries=Retry(total=4, backoff_factor=1, status_forcelist=[429, 500, 502, 503, 504]))
)
...
def resolve_place(query: str) -> tuple[int, str]:
    """Place id + display name for an iNaturalist place id or search text ("Taiwan", "Yilan")."""
...
def _get(path: str, params: dict) -> dict:
    ...
    resp = _session.get(API + path, params=params, timeout=30)
